    ...
    constructor(methods, streams, options) {
        this.http = http.createServer((req, res) => {
            if (req.method === 'GET') {
                ...
                if (RpcServer._authenticate(req, res, config.username, config.password, /*raw*/ false)) {
        ...
        this.http.on('upgrade', async (req, socket, head) => {
            if (!RpcServer._authenticate(req, null, config.username, config.password, /*raw*/ true))
                socket.destroy();
        ...
        this._hijackWS = options && options.hijackWS;
