...
class RpcClient {
    ...
    constructor(url, options) {
        ...
        this._port = this._url.port || defaultPort;
    ...
    get httpURL() {
        return `${this._tls ? 'https' : 'http'}://${this.host}:${this.port}/`;
    }

    get wsURL() {
        return `${this._tls ? 'wss' : 'ws'}://${this.host}:${this.port}/`;
    }
    ...
    async _executeHttp(requestId, method, ...params) {
        ...
        if (!this._wsUnsupported && !this._wsAvailable) {
            if (await this._tryWithWs(headers)) {
    ...
    _executeWs(requestId, method, ...params) {
        return new Promise((resolve, reject) => {
            // Send request
            try {
                const body = JSON.stringify(makeRequest(requestId, method, ...params));
